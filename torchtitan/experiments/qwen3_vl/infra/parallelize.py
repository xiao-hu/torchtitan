...
def parallelize_qwen3_vl(
    ...
    if visual_compile_enabled:
        ...
        model.visual = torch.compile(model.visual, backend=job_config.compile.backend, fullgraph=False, dynamic=True)
        logger.info("Applied torch.compile (dynamic=True) to native vision encoder")
